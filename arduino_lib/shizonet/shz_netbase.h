		...
		scriptFunction("send", [](ShizoNetDevice* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				...
				if (params.size() >= 2)
					send_id = var_to_device(cmd, params[1], obj->m_device.get(), sequential, timeout);
				else
					send_id = obj->m_device.get()->send_reliable(cmd,
						0,
						0,
						SHZNET_PKT_FMT_DATA, sequential, timeout);
...
class ShizoNetBase : public shzobject_ext<ShizoNetBase>, public shznet_server
{
	...
	shzvector<shzptr<shzblock>> m_connect_cbs;
	shzvector<shzptr<shzblock>> m_artnet_connect_cbs;

	shzvector<shzptr<shzblock>> m_artnet_recv_cbs;
	std::vector<std::pair<bool, shzvar_shared>> m_artnet_recv_buffer;
	bool m_artnet_received = false;
	...
	{
		shznet_server::on_device_disconnect(dev_info, reason);
	};

	void on_artnet_receive(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data, shznet_ip remoteIP)
	{
		if (universe >= 16)
			return;

		if (!m_artnet_recv_buffer.size())
		{
			m_artnet_recv_buffer.resize(16);
			for (auto& it : m_artnet_recv_buffer)
			{
				it.first = false;
				it.second->get_uchar_array()->resize(512);
			}
		}

		m_artnet_recv_buffer[universe].first = true;
		m_artnet_recv_buffer[universe].second->get_uchar_array()->resize(512);
		memcpy(m_artnet_recv_buffer[universe].second->get_uchar_array()->data(), data, length);
		m_artnet_received = true;
	}
...
public:
	...
	ShizoNetBase(shzscript* s = 0, shzstring name = "DEFAULT", short port = ART_NET_PORT)
	{
		...
		setArtDmxCallback([this](uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data, shznet_ip remoteIP)
			{
				on_artnet_receive(universe, length, sequence, data, remoteIP);
			});

		add_command("remote_log", [this](shznet_ip& adr, byte* data, size_t size, shznet_pkt_header& hdr)
			{
				auto dev = this->find_device(hdr.macid_source);
				if (!dev || !data)
					return;
...
#ifdef ARDUINO
		add_command_respond("run_prompt", [s](std::shared_ptr<shznet_responder> r)
			{
		...
		add_command_respond("reboot", [s](std::shared_ptr<shznet_responder> r)
			{
				ESP.restart();
			});
#endif
		add_command_respond("debug_log", [s](std::shared_ptr<shznet_responder> r)
			{
	...
	void scriptUpdate() override
	{
		...
		if (m_artnet_received)
		{
			m_artnet_received = false;
			for (uint32_t i = 0; i < 16; i++)
			{
				if (m_artnet_recv_buffer[i].first)
				{
					m_artnet_recv_buffer[i].first = false;
					shzvar* buffer_var = m_artnet_recv_buffer[i].second.get();

					for (auto it = m_artnet_recv_cbs.begin(); it != m_artnet_recv_cbs.end();)
					{
						auto func = it->get();
						if (!func)
						{
							it = m_artnet_recv_cbs.erase(it); // Erase the element and update the iterator
							continue;
						}

						auto p1 = func->getParam(0);
						auto p2 = func->getParam(1);
						if (p1) p1->initInt(i);
						if (p2) p2->initRef(buffer_var);
						func->run();
						++it; // Move to the next element
					}
				}
	...
	static void scriptRegister()
	{
		...
		scriptFunction("on_connect", [](ShizoNetBase* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				...
				obj->m_connect_cbs.push_back(fn);
		...
		scriptFunction("on_artnet_connect", [](ShizoNetBase* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				...
				obj->m_artnet_connect_cbs.push_back(fn);
		...
		scriptFunction("on_artnet", [](ShizoNetBase* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
				...
				obj->m_artnet_recv_cbs.push_back(fn);
		...
		scriptFunction("send_osc", [](ShizoNetBase* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() < 2)
					return;
				...
				auto ip = params[0]->get_string_cptr();
				auto jh = params[1]->get_json();
				auto port = params.size() >= 3 ? params[2]->get_int() : 8000;
				
				auto pkt = obj->osc_client_create();
				...
				{					
					if (!it->is_json())
					{
						pkt.openMessage(it->key.get().c_str(), 1);

						if (it->is_string())
						{
							pkt.string(it->get_string_cptr());
						}
						else if (it->is_int())
						{
							pkt.int32(it->get_int());
						}
						else if (it->is_float())
						{
							pkt.float32(it->get_float());
						}							
					}
					else
					{
						auto sub_jh = it->get_json();

						pkt.openMessage(it->key.get().c_str(), sub_jh->jsons.size());

						for (auto sub_json : sub_jh->jsons)
						{
							if (sub_json->is_string())
							{
								pkt.string(sub_json->get_string_cptr());
							}
							else if (sub_json->is_int())
							{
								pkt.int32(sub_json->get_int());
							}
							else if (sub_json->is_float())
							{
								pkt.float32(sub_json->get_float());
							}
							else
								SLH_Instance()->logerror("variable type not valid for OSC.");
						}

						//TODO function: get_json_size calculate total size + array size?
						//Or no arrays yet?

						// Add a message with two arguments and an array with 6 elements;
						// for efficiency this needs to be known in advance.
						//pkt.openMessage(*it->key, 2 + OSCPP::Tags::array(6));
						SLH_Instance()->logerror("arrays not supported yet!");
						continue;
					}
					pkt.closeMessage();
				}
				...
				try
				{
					...
					for (size_t i = 0; i < ipBytes.size(); ++i) {
						...
						ipBytes[i] = static_cast<uint8_t>(std::stoi(token));
					}

					obj->osc_client_transmit(shznet_ip((void*)ipBytes.data(), (int16_t)port), pkt);
				}
				catch (const std::exception& e)
				{
					SLH_Instance()->logerror("Error while sending: %s", e.what());
				}

			}, 3, true, false, "(ip, cmd_json, port)");
#endif
	}
};
...
class ShizoNetClient : public shzobject_ext<ShizoNetClient>, public shznet_client
{
	SHZOBJECT_DECLARE(ShizoNetClient);
	...
	virtual void on_device_connect(shznet_device_ptr dev_info) override
	{
		SLH_Instance()->log("%s connected.", dev_info->get_name().c_str());

		shznet_client::on_device_connect(dev_info);
	...
	virtual void on_device_disconnect(shznet_device_ptr dev_info, const char* reason)
	{
		SLH_Instance()->log("%s disconnected (%s).", dev_info->get_name().c_str(), reason);

		shznet_client::on_device_disconnect(dev_info, reason);
	};
...
public:
	...
	static ShizoNetClient* global_client;

	ShizoNetClient(shzscript* s = 0, shzstring name = "DEFAULT", short port = ART_NET_PORT)
	{
		_has_shizoscript_json = true;
	...
	virtual ~ShizoNetClient()
	{
		if (global_client == this)
			global_client = 0;
	}
	...
	static void scriptRegister()
	{
		scriptFunction("enable_shizonet", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1)
					return;
		...
		scriptFunction("set_debug", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1)
					return;
		...
		scriptFunction("send_artnet_sync", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1)
					return;
		...
		scriptFunction("on_connect", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_artnet_connect", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_any_connect", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_artnet", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_command", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 2 || params[1]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_stream", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 2 || params[1]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_get", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 2 || params[1]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("on_remote_log", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("set_name", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1)
					return;
		...
		scriptFunction("get_mac", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.initString(obj->local_mac().str().c_str());

			}, 0, true, false, "()");

		scriptFunction("get_ip", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.initString(obj->local_ip().str().c_str());
		...
		//IMPLEMENT SCRIPT FUNCTION THAT EXTERNAL FUNCTIONS CAN RETURN ARRAYS !!!
		scriptFunction("for_each_device", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("for_each_artnet_device", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1 || params[0]->Type != SHZVAR_FUNC)
					return;
		...
		scriptFunction("clear_artnet_buffers", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				for (auto it : obj->m_artnet_devices)
				{
					it.second->clear_artnet_buffer(params.size() == 1 ? params[0]->get_int() : 1);
		...
		scriptFunction("clear_all_buffers", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				for (auto it : obj->m_artnet_devices)
				{
					it.second->clear_artnet_buffer(params.size() == 1 ? params[0]->get_int() : 1);
		...
		scriptFunction("get_devices", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.get_json()->clear();
		...
		scriptFunction("get_artnet_devices", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.get_json()->clear();
		...
		scriptFunction("get_all_devices", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.get_json()->clear();
		...
		scriptFunction("get_offline_devices", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.get_json()->clear();
		...
		scriptFunction("get_device_mac", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1)
					return;
				auto mac = params[0]->get_string();
		...
		scriptFunction("get_artnet_device_mac", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() != 1)
					return;
				auto mac = params[0]->get_string();
		...
		scriptFunction("get", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				result.initInt(0);
		...
		scriptFunction("artnet_sync", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				obj->artnet_sync_now();
...
#ifdef SHZNET_OSC_SUPPORT
		scriptFunction("send_osc", [](ShizoNetClient* obj, shzblock* blk, shzvector<shzvar*>& params, shzvar& result)
			{
				if (params.size() < 2)
					return;
...
SHZOBJECT_INSTANCE(ShizoNetClient, "shizonet_client");
ShizoNetClient* ShizoNetClient::global_client;
...
shz_module_loader loader_netbase([](shzscript* s)
	{
		s->addExtClass<ShizoNetBase>("shizonet_base", [s](shzblock* blk, shzvector<shzvar*>& params, void* usrdata, shzvar& result)
			{
				...
				if (!tmp)
				{
					tmp = new ShizoNetBase(s, params.size() >= 1 ? params[0]->get_string() : "DEFAULT", params.size() >= 2 ? params[1]->get_int() : ART_NET_PORT);
		...
		s->addExtClass<ShizoNetBase>("shizonet_client", [s](shzblock* blk, shzvector<shzvar*>& params, void* usrdata, shzvar& result)
			{
				ShizoNetClient* tmp = ShizoNetClient::global_client;
				if (!tmp)
				{
					tmp = new ShizoNetClient(s, params.size() >= 1 ? params[0]->get_string() : "DEFAULT", params.size() >= 2 ? params[1]->get_int() : ART_NET_PORT);
					s->register_update(shzptr<shzobject>(tmp));
					s->register_update_flush(shzptr<shzobject>(tmp));
					tmp->set_node_name(params.size() >= 1 ? params[0]->get_string().c_str() : "DEFAULT");
				}
...
shznet_server* shz_netbase_create(shzscript* s, std::function<bool(const char*)> run_script_cb)
{
	...
	return ShizoNetBase::global_base;
}
shznet_client* shz_netclient_create(shzscript* s, std::function<bool(const char*)> run_script_cb)
{
	...
	if (run_script_cb)
	{
		ShizoNetClient::global_client->add_command_respond("run_script", [s, run_script_cb](std::shared_ptr<shznet_responder> r)
			{
				Serial.println("Run_script()");
				if (r->format() == SHZNET_PKT_FMT_STRING)
					run_script_cb((const char*)r->data());
	...
	return ShizoNetClient::global_client;
}
#endif
